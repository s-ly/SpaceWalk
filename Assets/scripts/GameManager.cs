...
public class GameManager : MonoBehaviour {
  ...
  [SerializeField] GameObject StoreButtonMission;
  ...
  public void OpenDialogMission() {
    ...
    if (StoreButton.activeSelf) {
      StoreButton.SetActive(false); // скрыть кнопку магазина
      StoreButtonMission.SetActive(false);
    }
    if (Store.activeSelf) {
  ...
  public void CloseDialog() {
    ...
    if (SpacePodZone.activeSelf) {
      StoreButton.SetActive(true); // показать кнопку магазина
